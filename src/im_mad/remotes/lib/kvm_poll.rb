...
require_relative 'poll_common'
require_relative 'probe_db'
...
module KVM
    ...
    def self.all_vm_info
        ...
        vms.each do |name, vm|
            ...
            state  = get_state(dominfo['State'], vm[:reason])
            ...
            if state == 'RUNNING' && (vm[:reason] != 'migrating')
    ...
    def self.get_state(state, reason = 'missing')
        ...
        when 'paused'
            ...
            when 'migrating'
                'RUNNING'
            when 'I/O error', 'watchdog', 'crashed', 'post-copy failed', 'unknown'
                'FAILURE'
            when 'user'
                'SUSPENDED'
            else
